...
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView, StatusBar, Modal, ScrollView, Image, Alert } from 'react-native';
...
const DriverSearchScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeLoad = firebaseLoadsService.subscribeToLoad(loadId, (load) => {
      setCurrentLoad(load);
    ...
    const unsubscribeBids = firebaseLoadsService.subscribeToLoadBids(loadId, (bidsData) => {
      setBids(bidsData);
  ...
  useEffect(() => {
    const fetchInterestedDrivers = async () => {
      ...
      try {
        const driverPromises = bids.map(async (bid) => {
          try {
            ...
            if (driverResult.success) {
              return {
                id: bid.driverId,
                name: driverResult.data.name || driverResult.data.companyName || driverResult.data.displayName || 'Driver',
                rating: driverResult.data.rating || '4.5',
                deliveries: driverResult.data.completedDeliveries || 0,
                price: bid.offerAmount,
                time: bid.createdAt ? new Date(bid.createdAt).toLocaleTimeString() : 'Just now',
                bidId: bid.id,
        ...
        const driversData = (await Promise.all(driverPromises)).filter(driver => driver !== null);
      ...
      <View style={styles.bottomCard}>
        ...
        <Text style={styles.searchingSubtitle}>
          {isSearching
            ...
            : interestedDrivers.length > 0
              ? `${interestedDrivers.length} driver${interestedDrivers.length > 1 ? 's' : ''} interested`
              : isLoadingDrivers
        ...
        <View style={styles.driversRow}>
          <Text style={styles.driversCount}>
            {interestedDrivers.length > 0
              ? `${interestedDrivers.length} driver${interestedDrivers.length > 1 ? 's' : ''} considering your load`
        ...
        <Text style={styles.fareOffer}>
          Your load fare: ₦{currentLoad?.fareOffer?.toLocaleString() || '10,000'}
        </Text>
        ...
        >
          <View style={styles.driverAvatar}>
            <Text style={styles.driverAvatarText}>
              {driver.avatar ? '👤' : driver.name.split(' ').map(word => word[0]).join('')}
            </Text>
          </View>
          <View style={styles.driverInfo}>
            <View style={styles.driverHeader}>
              <Text style={styles.driverName}>{driver.name}</Text>
              <Text style={[styles.bidStatus, { color: getBidStatusColor(driver.bidStatus) }]}>
            ...
            <Text style={styles.driverPrice}>₦{driver.price.toLocaleString()}</Text>
          </View>
          <View style={styles.driverActions}>
              ...
              <>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    Alert.alert('Decline Bid', `Decline bid from ${driver.name}?`, [
                      { text: 'Cancel', style: 'cancel' },
                      {
                        ...
                        onPress: async () => {
                          try {
                            await firebaseLoadsService.rejectBid(driver.bidId, loadId);
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    try {
                      await firebaseLoadsService.acceptBid(driver.bidId, loadId, driver.id);
                      Alert.alert('Success', `Accepted bid from ${driver.name}!`, [
                        { text: 'OK', onPress: () => navigation.navigate('DriverFound', { loadId, load: currentLoad }) }
                      ]);
                    } catch (error) {
            ...
            {driver.bidStatus === 'accepted' && (
              <Text style={[styles.statusText, { color: '#4CAF50' }]}>ACCEPTED</Text>
            )}
            {driver.bidStatus === 'rejected' && (
              <Text style={[styles.statusText, { color: '#FF0000' }]}>REJECTED</Text>
            )}
          </View>
        </View>
      ))}
      ...
      {interestedDrivers.length === 0 && searchProgress === 100 && (
        <View style={styles.noDriversCard}>
          ...
          <Text style={styles.noDriversMessage}>
            No drivers have shown interest in your load yet. This could take a few minutes.
          </Text>
          <TouchableOpacity
