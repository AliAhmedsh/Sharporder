...
export const firebaseLoadsService = {
  ...
  subscribeToLoad: (loadId, callback) => {
    ...
    const unsubscribe = onSnapshot(loadRef, (docSnapshot) => {
      // Handle both function and boolean cases for exists
      const docExists = typeof docSnapshot.exists === 'function'
        ? docSnapshot.exists()
        : docSnapshot.exists;
  ...
  getDriverById: async (driverId) => {
    try {
      ...
      const docExists = typeof driverDoc.exists === 'function'
        ? driverDoc.exists()
        : driverDoc.exists;
